...
turnMethods = {
	...
	zoom: function(newZoom) {
		...
		if (typeof(newZoom)=='number') {
			...
			var size = $(this).turn('size'),
				iz = 1/data.zoom,
				newWidth = size.width * iz * newZoom,
				newHeight = size.height * iz * newZoom;
	...
	_released: function(e, point) {
		...
		{
			e.preventDefault();
			turnMethods._turnPage.call(data.opts.turn, data.opts.next, true);
		}
		...
		if (data.pageMv.length && data.pageMv[0]!==0) {
			...
			for (page in data.pageWrap) {
				...
				data.pageWrap[page].css({
					...
					zIndex: pos.pageZ[page] || ((fixed) ? -1 : 0)
				});
...
flipMethods = {
	...
	_cornerActivated: function(e) {
		...
		e = (isTouch && e.originalEvent) ? e.originalEvent.touches : [e];
	...
	_fold: function(point) {
		...
		switch (data.effect) {
			...
			case 'hard':
				...
				if (half) {
					relX = -relX+1;
					data.wrapper.css({zIndex: totalPages});
					data.fpage.css({zIndex: totalPages-1});
				} else {
					relX = relX-1;
					data.wrapper.css({zIndex: totalPages-1});
					data.fpage.css({zIndex: totalPages});
				}
	...
	_moveFoldingPage: function(move) {
		...
		if (move) {
			...
			if (place[nextPage]!=data.opts.page) {
				...
				flipMethods.setData.call(this, {folding: nextPage});
			...
			if (data.folding) {
				...
				// If we have a flipbook available
